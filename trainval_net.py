...
def parse_args():
  ...
  parser.add_argument('--cag', dest='class_agnostic',
                      help='whether perform class_agnostic bbox regression',#是否使用类不可知的box回归，True：box回归预测每个roi
                      # 只预测4个值  False：box回归预测时，每个roi针对每个类别预测4个回归值，如果类别数为20，那么预测20*4个回归值
                      action='store_true')
...
class sampler(Sampler):
  def __init__(self, train_size, batch_size):#train_size=训练图片的数量(10022)  batch_size=b(3)
...
if __name__ == '__main__':
  ...
  imdb, roidb, ratio_list, ratio_index = combined_roidb(args.imdb_name)#imdb_name=voc_2007_trainval
  train_size = len(roidb)#10022

  # ratio_list[ratio_index]将各个图片的的长宽比率按照从小到大排序（根据ratio_index排好的）
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      im_info.sahpe=(b,3)
      gt_boxes.sahpe=(b,20,5)一应该是这张图上有20个gt，5分别为4个坐标加一个类别
      num_boxes = (b)
      '''
      ...
      if step % args.disp_interval == 0:
        ...
        #session的含义还未知，epoch就是数据集迭代多少次训练  iters_per_epoch是遍历一遍数据集需要多少次迭代（psacal ：数据集里共10020张图片，batch=3 那么iters_per_epoch=10020/3=3340
        # step就是在这个epoch中进行了多少次batch训练（共10020/3=3340次））
