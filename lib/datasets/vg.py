...
class vg(imdb):
    ...
    def evaluate_detections(self, all_boxes, output_dir):
        ...
        :param all_boxes:  #最终all_boxes.shape = (21, n, h, 5)  这里all_boxes是一个列表，里面的h有不同的值，最多有20*n（不算背景类）个值，代表每张图片每个类检测出来的box个数
        :param output_dir: Output/default 文件夹（不确定是不是这个文件夹）
        :return:
        '''
    ...
    def _write_voc_results_file(self, classes, all_boxes, output_dir):
        ...
        :param classes:
        :param all_boxes: #最终all_boxes.shape = (21, n, h, 5)  这里all_boxes是一个列表，里面的h有不同的值，最多有20*n（不算背景类）个值，代表每张图片每个类检测出来的box个数
        :param output_dir: output/default
        :return:
        '''
        for cls_ind, cls in enumerate(classes):
            #cls_ind=0,1,2,...20  cls='__background__','A','B',....
            if cls == '__background__':
                continue
            print('Writing "{}" vg results file'.format(cls))
            filename = self._get_vg_results_file_template(output_dir).format(cls)##'detections_' + self._image_set + '_{:s}.txt'
