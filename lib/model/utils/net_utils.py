...
def _smooth_l1_loss(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
    ...
    sigma_2 = sigma ** 2                            #1*1=1
    box_diff = bbox_pred - bbox_targets             #shape=(b*128,4)预测的回归值和回归target之间的差值
    in_box_diff = bbox_inside_weights * box_diff    #将负样本和ignore的差值设为0  shape=(b*128,4)
    abs_in_box_diff = torch.abs(in_box_diff)        #取绝对值  shape=(b*128,4)
    smoothL1_sign = (abs_in_box_diff < 1. / sigma_2).detach().float()  #shape=(b*128,4)值为0或1  也就是将差值小于1的标记为1，否则标记为0

    in_loss_box = torch.pow(in_box_diff, 2) * (sigma_2 / 2.) * smoothL1_sign \
                  + (abs_in_box_diff - (0.5 / sigma_2)) * (1. - smoothL1_sign)#这部分看smoothL1 loss的公式就能看懂
    #in_loss_box.shape=(b*128,4) 也就是损失函数求和的部分（这里还没有求和，以向量的形式计算）
    out_loss_box = bbox_outside_weights * in_loss_box  #shape=(b*128,4)
    loss_box = out_loss_box #shape=(b*128,4)
    for i in sorted(dim, reverse=True): #dim=[1]   i=1
      loss_box = loss_box.sum(i)
    #依次沿着第一维度求和  得到#shape=(b*128)  也就是将每张图片每个roi的四个回归值的loss加起来
    loss_box = loss_box.mean() #将各个图片各个roi的loss求平均 是一个值
    return loss_box
    ...
    sigma_2 = sigma ** 2#3*3=9
    box_diff = bbox_pred - bbox_targets#shape=(b,4*9,w,h)预测的回归值和回归target之间的差值
    in_box_diff = bbox_inside_weights * box_diff#将负样本和ignore的差值设为0  shape=(b,4*9,w,h)
    abs_in_box_diff = torch.abs(in_box_diff)#取绝对值  shape=(b,4*9,w,h)
    smoothL1_sign = (abs_in_box_diff < 1. / sigma_2).detach().float()  #shape=(b,4*9,w,h)值为0或1  也就是将差值小于1/9的标记为1，否则标记为0
    '''
    abs_in_box_diff = torch.Tensor([1,5,3,4,8])
    smoothL1_sign = (abs_in_box_diff < 4).detach().float()
    print(smoothL1_sign)=tensor([1., 0., 1., 0., 0.])
    ...
    for i in sorted(dim, reverse=True): #dim=[1,2,3]   i=3,2,1
      loss_box = loss_box.sum(i)
    #依次沿着第三，第二，第一维度求和  得到#shape=(b)  [loss1,loss2,....lossb] 也就是一个batch里面各个图片的loss
    loss_box = loss_box.mean() #将各个图片loss求平均 是一个值
    return loss_box
